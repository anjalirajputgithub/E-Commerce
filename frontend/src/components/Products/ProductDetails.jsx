  ...
  useEffect(() => {
   ...
   const handleAddToCart = () => {
    if(!selectedSize || !selectedColor){
      toast.error("Please select the size and color before adding to cart", {duration: 1000});
      return;
    }
  ...
  return (
    <div className='p-6'>
      {selectedProduct && (
        ...
        <div className='flex flex-col md:flex-row h-500'>
            {/* LEFT THUMBNAIL */}
            <div className='hidden md:flex flex-col space-y-4 mr-6'>
                {selectedProduct.images.map((image, index)=>(
                    <img src={image.url} alt={image.altText || `Thumbnail ${index}`} key={index} 
            ...
            <div className='md:w-1/2'>
             <div className='mb-4'>
              <img src={mainImage} alt="Main Product" className='w-auto h-auto object-cover rounded-lg'/>
             </div>
            </div>
            {/* Mobile Thumnail */}
            <div className='md:hidden flex overscrol-x-scroll space-x-4 mb-4'>
              {selectedProduct.images.map((image, index)=>(
                <img src={image.url} alt={image.altText || `Thumbnail ${index}`} key={index} 
                  className={`w-20 h-20 object-cover rounded-lg cursor-pointer border ${mainImage === image.url ? "border-black" : "border-gray-300"}`}
                  onClick = {()=> setMainImage(image.url)}/>
            ...
            <div className='md-w-1/2 md:ml-10'>
               ...
               onClick={handleAddToCart} className={`bg-black text-white py-2 px-6 rounded w-full mb-4 ${isButtonDisabled ? "cursor-not-allowed opacity-50" : "hover:bg-gray-900"}`}>
