...
const NewArrival = () => {
  ...
  const handleMouseMove = (e) => {
    ...
    const x = e.pageX - scrollRef.current.offset;
    const walk = x- startX;
    ...
    <section className='py-16 px-4 lg:px-0'>
      ...
      ${isDragging ? "cursor-grabbing" : "cursor-grab"}`}
        ...
        {newArrivals.map((product)=> (
          <div key={product._id} className='min-w-[100%] sm:min-w-[50%] lg:min-w-[30%] px-4 relative'>
            <img src={product.images[0]?.url} alt={product.images[0]?.altText || product.name} 
            className='w-full h-[350px] object-cover rounded-lg' draggable="false"/>
            <div className='bg-gray-900 bg-opacity-50 backdrop-blur-md text-white p-4 rounded-b-lg'>
